...
var createChannelTransaction = function(opts) {
  ...
  //if a change address isn't given, then we must conclude that the "toAddress" should be receiving
  //all of the inputs funds minus the miner fee. The caller should take care to include a
  //change address if all the funds aren't intended for the toAddress.
  if (opts.changeAddress) {
    chanTx.change(new Address(opts.changeAddress, network));
    chanTx.to(toAddress.toString(), satoshis);
    chanTx.fee(fee);
  } else {
    chanTx.to(toAddress.toString(), (utxo.satoshis - fee));
  }
  ...
  chanTx.sign(consumerPrivateKey);
  return chanTx;
};
...
var createCommitmentRefundTransaction = function(opts) {
  ...
  //this makes the tx non-final, but also prevents RBF
