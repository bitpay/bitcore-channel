...
var createChannelTransaction = function(opts) {
  ...
  if (!opts.redeemScript) {
    throw new Error('redeemScript is required');
  }
  if (!(opts.fee > 0)) {
    throw new Error('fee is required');
  }
  if (!opts.commitmentTransaction) {
    throw new Error('commitment transaction is required');
  }
  if (typeof opts.changeAddress === 'undefined') {
    throw new Error('change address is required to be explicitly null or a value, but not undefined');
  }
  ...
  if (commitmentTransactionOutputIndex < 0) {
    throw new Error('could not locate proper output in supplied commitment transaction');
  }
  ...
  if (changeAmount < 0) {
    throw new Error('Output amounts exceed input amounts');
  }

  if (changeAmount !== 0 && changeAddress === null) {
    throw new Error('change address was explicitly set to null, however the sum of the outputs plus the fee does not exactly equal the inputs amounts');
  }
...
var createCommitmentTransaction = function(options) {
  ...
  var network = opts.network || 'livenet';
  if (!opts.privateKey) {
    throw new Error('private key needed for spending transaction');
  }
  if (!opts.satoshis) {
    throw new Error('bitcoin amount in satoshis is needed');
  }
  ...
  if (!opts.redeemScript) {
    throw new Error('redeem script is required');
  }
  if (!(opts.fee > 0)) {
    throw new Error('fee is required');
  }
  if (!opts.prevTx) {
    throw new Error('previous transaction is required');
  }
  if (!(opts.prevTxOutputIndex >= 0)) {
    throw new Error('previous transaction output index is required');
  }
