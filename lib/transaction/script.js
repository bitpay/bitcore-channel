...
Script.buildCLTVRedeemScript = function(pubkeys, lockTime) {
  ...
  script
    ...
    .add(Opcode.OP_CHECKSIG) // 1
    .add(Opcode.OP_IF) // 2 if consumer sig not valid, do nothing else
    .add(Opcode.OP_DEPTH) // 3 how many more stack items?
    .add(Opcode.OP_IF) // 4 if at least one stack item, check provider sig
    .add(new Buffer(pubkeys[0], 'hex')) // 5
    .add(Opcode.OP_CHECKSIG) // 6
    .add(Opcode.OP_ELSE) // 7 there are no stack items, therefore check lock
    .add(Script._convertExpirationTimeStampToBuffer(lockTime)) // 8
    .add(Opcode.OP_CHECKLOCKTIMEVERIFY) // 9 if true, leave the lock time on the stack as the only item
    .add(Opcode.OP_ENDIF) // 10
    .add(Opcode.OP_ENDIF) // 11
