...
function Refund(opts) {
  ...
  this.refundAddress = opts.refundAddress instanceof bitcore.Address
    ...
    if (!this.refundAddress.data) {
      console.trace();
    console.log(this.refundAddress);
    }
...
Refund.prototype.isSigned = function() {
  return this.builder.build().isComplete();
};
...
Refund.prototype.build = function() {
  ...
  $.checkState(result.isComplete(), 'Transaction must be fully signed!');

  return result.serialize().toString('hex');
};
