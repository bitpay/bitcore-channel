...
function Consumer(opts) {
  ...
  if (opts.refundAddress) {
    if (opts.refundAddress instanceof bitcore.Address) {
      ...
      this.refundAddress = new bitcore.Address(opts.refundAddress, this.network);
    }
  } else {
    /**
     * @type bitcore.Key
...
Consumer.prototype.setupServerPublicKey = function setupServerPublicKey(serverPublicKey) {
  assert.ok(util.isCompressedPubkey(serverPublicKey), 'Public key provided is not compressed hexa');
  assert.ok(_.size(serverPublicKey) === 66, 'Public key provided is not compressed');
  /**
...
Consumer.prototype._createCommitmentTx = function _createCommitmentTx() {
  assert.ok(this.serverPublicKey, 'Must set up a server public key first');
  assert.ok(_.size(this.inputs), 'Must have at least one input stored to establish channel');
...
Consumer.prototype._inputFromCommitment = function _inputFromCommitment() {
  ...
  hash.copy(output);
  output.writeUInt32LE(this.amount - STANDARD_FEE, hash.length);
...
Consumer.prototype._outputToConsumer = function _outputToConsumer() {
  assert.ok(this.refundTx, 'Refund transaction must exist!');
...
Consumer.prototype.getPaymentTxSignature = function getPaymentTxSignature() {
  ...
  assert.ok(this.commitmentTx, 'The commitment transaction must be created before paying');
...
Consumer.prototype.getSignedRefundTx = function getSignedRefundTx() {
  assert.ok(this.serverRefundSignature, 'Refund transaction must be signed by server!');
  assert.ok(this.refundTx, 'Refund transaction was never built!');
