...
Consumer.prototype.setupServerPublicKey = function setupServerPublicKey(serverPublicKey) {
  $.checkArgument(util.isCompressedPubkey(serverPublicKey), 'Public key provided is not compressed hexa');
  $.checkArgument(_.size(serverPublicKey) === 66, 'Public key provided is not compressed');
  /**
...
Consumer.prototype._createCommitmentTx = function _createCommitmentTx() {
  ...
  $.checkState(_.size(this.inputs), 'Must have at least one input stored to establish channel');
...
Consumer.prototype._outputToConsumer = function _outputToConsumer() {
  $.checkState(this.refundTx, 'Refund transaction must exist!');
...
Consumer.prototype.getPaymentTxSignature = function getPaymentTxSignature() {
  ...
  $.checkState(this.commitmentTx, 'The commitment transaction must be created before paying');
...
Consumer.prototype.getSignedRefundTx = function getSignedRefundTx() {
  $.checkState(this.serverRefundSignature, 'Refund transaction must be signed by server!');
