...
var _findInvalidInputs = function(tx) {
  ...
  //If, for some reason, we are looking for invalid inputs AFTER the CHECKLOCKTIMEVERIFY
  //checks evaluate to true (after the locktime/block), then we can still spend the channel tx
  //provided the consumer doesn't beat us to the punch. In other words, we should never
  //be in this position as a provider. If we are, we want to fail verification with the
  //intention of human intervention.
  var inputs = [];
  for(var i = 0; i < tx.inputs.length; i++) {
    var input = tx.inputs[i];
...
var _getRedeemScriptFromInput = function(input) {
  return input.script.chunks[input.script.chunks.length-1].buf.toString('hex');
};
...
var _checkCommitmentRedeemScript = function(redeemScript, input) {
  var inputRedeemScriptHex = _getRedeemScriptFromInput(input);
 ...
 * safety.
*/
var signTx = function(options) {

  if (!options.privateKey ||
      !options.commitmentTransaction ||
      !options.channelTx) {
        throw 'missing parameters';
  }
  ...
  var commitmentTx = new Transaction(options.commitmentTransaction);
  var channelTx = new CLTVTransaction(options.channelTx);
  var privateKey = new PrivateKey(options.privateKey, network);
  var invalidInputs = _findInvalidInputs(channelTx); //this should find all the invalid inputs on the channel tx

  //there should be 1 and only 1 invalid input and it should be the commitment tx's utxo.
  if (invalidInputs.length !== 1) {
    throw 'provided channel transaction does not contain an unsigned utxo from the commitment transaction';
  }

  var commitmentTxInput = invalidInputs.input;
  var redeemScript = _getRedeemScriptFromInput(commitmentTxInput);
  var commitmentTxOutputIndex = utils.findCommitmentTransactionOutputIndex(redeemScript, commitmentTx);

  if (commitmentTxOutputIndex < 0) {
    throw 'could not locate the appropriate utxo from the provided commitment transaction for the given channel transaction';
  }

  var utxo = new Output({
    satoshis: commitmentTx.outputs[commitmentTxOutputIndex].satoshis,
    script: commitmentTx.outputs[commitmentTxOutputIndex].toHex(),
  });

  commitmentTxInput.output = utxo;
