...
var signChannelTransaction = function(options) {
  ...
  if (!channelTx.serialize()) {
    throw new Error('channel tx did not verify after signing');
  }
...
var increaseChannelTransactionFee = function(options) {
  if ((options.feeIncreaseSatoshis <= 0) ||
      !options.channelTx ||
      !(options.outputIndex >= 0)) {
        throw new Error('missing parameters');
  }

  var feeIncreaseSatoshis = options.feeIncreaseSatoshis;
  var channelTx = new CLTVTransaction(options.channelTx);
  var outputIndex = options.outputIndex;

  channelTx.outputs[outputIndex].satoshis -= feeIncreaseSatoshis;
  return channelTx;
}
